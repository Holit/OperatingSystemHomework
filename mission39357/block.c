...
/*
在已经完成的 SCAN 算法源代码的基础上进行改写，将请求队列分成若干个长度为 N 的子队列，调度
程序按照 FCFS 原则依次处理这些子队列，而每处理一个子队列时，又是按照 SCAN 算法
*/
...
IopDiskSchedule(
	...
	)
 {
 	...
 	ULONG OutwardShortestDistance  = 0xFFFFFFFF;
	
	//使用数组记录双向请求
	//规定：
	//	pNextRequest[1]为Offset > 0 方向
 	PREQUEST pNextRequest[2] = { NULL , NULL };

 	//请求队列分成若干个长度为 N 的子队列，调度程序按照 FCFS 原则依次处理这些子队列
 	static ULONG lenSubQueue = SUB_QUEUE_LENGTH;

 	if(0 == lenSubQueue){
 		lenSubQueue = SUB_QUEUE_LENGTH;
 	}
 	...
 	for (pListEntry = RequestListHead.Next;	// 请求队列中的第一个请求是链表头指向的下一个请求。
 		...
 		if (0 == Offset) {
 			...
 			// 记录向外移动距离最短的线程
 			//
 			if (-Offset < OutwardShortestDistance) {
 		
 				OutwardShortestDistance = -Offset;
